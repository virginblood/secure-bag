...
var (
	...
	// fields.
	AllProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), new(EthashConfig), nil}
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Diehard %v Gotham %v Engine: %v}",
		c.ChainId,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) IsDiehard(num *big.Int) bool {
	return isForked(c.DiehardBlock, num)
}

func (c *ChainConfig) IsGotham(num *big.Int) bool {
	return isForked(c.GothamBlock, num)
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.DiehardBlock, newcfg.DiehardBlock, head) {
		return newCompatError("Diehard fork block", c.DiehardBlock, newcfg.DiehardBlock)
	}
	if isForkIncompatible(c.GothamBlock, newcfg.GothamBlock, head) {
		return newCompatError("Gotham fork block", c.GothamBlock, newcfg.GothamBlock)
	}
	return nil
}
...
func (c *ChainConfig) Rules(num *big.Int) Rules {
	...
	return Rules{ChainId: new(big.Int).Set(chainId), IsHomestead: c.IsHomestead(num), IsEIP150: c.IsEIP150(num), IsEIP155: c.IsEIP155(num), IsEIP158: c.IsEIP158(num), IsByzantium: c.IsByzantium(num), IsDiehard: c.IsDiehard(num), IsGotham: c.IsGotham(num)}
}
